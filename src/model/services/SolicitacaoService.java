...
public class SolicitacaoService {
    ...
    public List<SolicitacoesAnalisadas> getSolicitacoesVisiveisParaUsuario(Usuario usuario) throws SQLException {
        if (permissaoService.visualizarTodasSolicitacoes(usuario)) {
            return solicitacaoDAO.select(StatusSolicitacao.PENDENTE, usuario.getUserType().getId(),
                    usuario.getId());
        }
    ...
    public List<SolicitacoesAnalisadas> getSolicitacoesPorStatus(StatusSolicitacao status,
                                                                 int idTipoUsuario, int idUser)
            throws SQLException {
        return solicitacaoDAO.select(status, idTipoUsuario, idUser);
    }

    public void atualizarStatusSolicitacao(Usuario usuario, PermissaoService permissaoService,
                                           SolicitacoesEnviadas solicitacao, StatusSolicitacao novoStatus)
            throws SQLException {
        if (permissaoService.aprovarReprovarSolicitacoes(usuario)) {
            ...
            Alert alert = new Alert(Alert.AlertType.WARNING,
                    "Acesso negado: Usuário não tem permissão para aprovar ou reprovar solicitações.");
            alert.show();
        }
    }
}